...
def getHolidays(year):
    ...
    holidays = list(set(list(map(lambda x: year+'-'+dict1[x[0]]+"-"+x[1], list(map(lambda x: x.text.split(), list(
        map(lambda y: y.find("td", attrs={"style": "white-space:nowrap;"}), date_table_data))))))).union(d))
    holidays.sort()
    print(holidays)
    return holidays
